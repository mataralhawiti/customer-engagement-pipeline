...
from . import config  # This ensures config.py runs and sets environment
from .config import REDIS_CONFIG, BIGQUERY_CONFIG, THIRD_PARTY_API_CONFIG, DEFAULT_EMIT_INTERVAL_SECS, PIPELINE_CONFIG
...
from .utils import _parse_json, _only_after, _event_ts_iso_to_epoch_seconds, to_redis_kv, load_bigquery_schema
...
def build_pipeline(
    ...
    content_after = (
        ...
        | "StampContentEventTime" >> beam.Map(
            lambda rec: beam.window.TimestampedValue(
                rec, _event_ts_iso_to_epoch_seconds(rec.get("publish_ts", "2024-01-01T00:00:00Z"))
            )
        )
    ...
    enrich_outputs = merged | "StatefulEnrich" >> beam.ParDo(
        ...
        EnrichWithContentStateful.CACHE_NEW_TAG,
        main="enriched"
    )
    ...
    _ = (
        cache_new
        | "CacheLog_WindowFix" >> beam.WindowInto(
            window.GlobalWindows(),
    ...
    _ = (
        ...
        | 'WriteToRedis' >> beam.ParDo(WriteToRedisDoFn(
    ...
    _ = (
        emitted
        | "SendToThirdPartyAPI" >> beam.ParDo(HTTPSenderDoFn(
