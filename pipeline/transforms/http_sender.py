...
class HTTPSenderDoFn(beam.DoFn):
    ...
    def process(self, element):
        ...
        try:
            ...
            self._errors.inc()
            self._log.error(f"Failed to send event to API: {e}, event_id: {element.get('id')}")
    ...
    def _send_to_api(self, event_batch: Dict[str, List[Dict[str, Any]]]):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self._log.info(f"Successfully sent {event_count} events to {url}. "
                             f"Response: {response_data.get('message', 'OK')}")
            else:
                self._log.warning(f"API returned status {response.status_code} for {event_count} events. "
...
class SendToThirdPartyAPI(beam.PTransform):
    ...
    def expand(self, pcoll):
        return (
            pcoll
            | "SendToAPI" >> beam.ParDo(HTTPSenderDoFn(
