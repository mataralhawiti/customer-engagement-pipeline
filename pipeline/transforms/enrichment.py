...
class EnrichWithContentStateful(DoFn):
    ...
    def process(
        ...
        if tag == "content":
            ...
            if raw:
                ...
                self.enriched_counter.inc()
                self.logger.info("Enriched %s event (id=%s) for content: %s", 
                               payload.get("event_type"), payload.get("id"), content_id)
                yield out
            else:
                ...
                self.logger.info("Buffered %s event (id=%s) for content: %s", 
                               payload.get("event_type"), payload.get("id"), content_id)
    ...
    def _enrich(evt: Dict[str, Any], content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if duration_ms is not None and isinstance(duration_ms, (int, float)) and duration_ms >= 0:
            engagement_seconds = round(duration_ms / 1000.0, 3)  # Round to 3 decimal places
        ...
        if (engagement_seconds is not None and 
            length_seconds is not None and 
            isinstance(length_seconds, (int, float)) and 
            length_seconds > 0):
            engagement_pct = (engagement_seconds / length_seconds) * 100
            out["engagement_pct"] = round(engagement_pct, 2)  # Round to 2 decimal places
        else:
