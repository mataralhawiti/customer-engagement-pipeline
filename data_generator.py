...
PODCAST_TITLES = [
    'Tech Talk Weekly', 'Startup Stories', 'Code & Coffee', 'Innovation Hub',
    'Digital Trends', 'Future Forward', 'Developer Diaries', 'AI Insights'
]
NEWSLETTER_TITLES = [
    'Morning Brief', 'Weekly Roundup', 'Industry Update', 'Market Watch',
    'Tech Digest', 'Startup News', 'Product Updates', 'Leadership Notes'
]
VIDEO_TITLES = [
    'Tutorial: Getting Started', 'Product Demo', 'Behind the Scenes',
    'Expert Interview', 'Case Study', 'Live Q&A', 'Workshop Recording'
]
...
class DataGenerator:
    ...
    def generate_content_row(self) -> Dict[str, Any]:
        ...
        slug = title.lower().replace(' ', '-').replace(':', '') + f"-{random.randint(1000, 9999)}"
    ...
    def run_generator(self, content_per_min: int, engagement_per_min: int, duration_mins: int, use_existing: bool = False, prefer_new: bool = False):
        ...
        print(f"  Total: {content_per_min * duration_mins} content, {engagement_per_min * duration_mins} engagement events")
        ...
        for minute in range(duration_mins):
            ...
            if content_per_min > 0:
                content_rows = [self.generate_content_row() for _ in range(content_per_min)]
            ...
            if engagement_per_min > 0:
                if use_existing:
                    ...
                    print(f"         Using {len(self.content_cache)} existing content items (--use-existing)")
                elif prefer_new and newly_created_content:
                    # Prefer newly created content if available
                    content_pool = newly_created_content
                    print(f"         Using {len(newly_created_content)} newly created content items (--prefer-new)")
                elif newly_created_content:
                    # Default behavior: use newly created if available
                    content_pool = newly_created_content
                    print(f"         Using {len(newly_created_content)} newly created content items for engagement")
                else:
                    # Fallback to existing content
                    content_pool = self.content_cache
                    print(f"         Using {len(self.content_cache)} existing content items for engagement")
                engagement_rows = [self.generate_engagement_row(content_pool) for _ in range(engagement_per_min)]
                engagement_rows = [row for row in engagement_rows if row is not None]
                if engagement_rows:
                    self.insert_engagement_batch(engagement_rows)
                    print(f"Minute {minute + 1}: Inserted {len(engagement_rows)} engagement events")
...
def main():
    ...
    parser.add_argument('--content', '-c', type=int, default=0, 
                       help='Content rows per minute (default: 0)')
    parser.add_argument('--engagement', '-e', type=int, default=10,
    ...
    try:
        ...
        with generator.connect_db() as conn:
    ...
    # Run generator
    generator.run_generator(args.content, args.engagement, args.duration, 
                          use_existing=args.use_existing, prefer_new=args.prefer_new)
